...
def build_forecasts(series: pd.Series, horizon: int = 12) -> Tuple[pd.DataFrame, pd.DataFrame]:
...
def plot_monthly(series: pd.Series, forecast: pd.DataFrame, metrics: pd.DataFrame) -> None:
    plt.rcParams.update({
        "font.size": 10,
        "axes.titlesize": 12,
        "axes.labelsize": 10,
    ...
    zoom_start = max(series.index.min(), series.index.max() - pd.DateOffset(years=4))
    zoom_series = series[series.index >= zoom_start]
    ax_zoom.fill_between(zoom_series.index, _millions(zoom_series.values), alpha=0.18, color="#3a3a3a")
    ax_zoom.plot(zoom_series.index, _millions(zoom_series.values), color="#2f2f2f", linewidth=2.0, label="Historical")
    ax_zoom.axvline(series.index.max(), linestyle=":", linewidth=1.4, color="#777777")
    _plot_forecast_lines(ax_zoom, series, forecast, include_legend=False)
    ax_zoom.set_title("Recent period + forecast zoom")
    ...
    if not metrics.empty:
        ...
        ax_table.set_title(f"Validation metrics\nBest: {best}", fontweight="bold")
    else:
        ax_table.text(0.5, 0.5, "No validation metrics", ha="center", va="center")
...
def main() -> None:
    ...
    forecast, metrics = build_forecasts(series, horizon=12)
    forecast.to_csv(OUT_FORECAST, index=False)
    metrics.to_csv(OUT_METRICS, index=False)
    plot_monthly(series, forecast, metrics)
