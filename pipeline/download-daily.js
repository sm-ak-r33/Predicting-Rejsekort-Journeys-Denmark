...
async function getVisiblePlusIcons(frame) {
  return await frame.evaluate(() => {
    return [...document.querySelectorAll(".expandIcon")]
      ...
      .map((el) => {
        const r = el.getBoundingClientRect();
        return { x: r.left + r.width / 2, y: r.top + r.height / 2, left: Math.round(r.left) };
      });
  });
}
...
async function clickTopVisibleYearPlus(frame, page) {
  ...
  const yearIcons = icons.filter((icon) => icon.left < 265).sort((a, b) => a.y - b.y);
  if (yearIcons.length === 0) throw new Error("No visible year-level plus icon found");
  await page.mouse.click(yearIcons[0].x, yearIcons[0].y);
  await page.waitForTimeout(2500);
}

async function clickVisibleMonthPlusesBottomUp(frame, page) {
  const icons = await getVisiblePlusIcons(frame);
  const monthIcons = icons.filter((icon) => icon.left >= 265).sort((a, b) => b.y - a.y);
...
async function exportExcel(page, frame, outputName) {
  ...
  if (!gridPos) throw new Error("VirtualGrid not found");
  ...
  if (!clickedExport) throw new Error("Export to Excel not found");
...
(async () => {
  ...
  await frame.evaluate(() => {
    const btn =
      ...
      [...document.querySelectorAll(".ObjectToolbarButton")].find((b) => b.className.includes("crosstab"));
    if (!btn) throw new Error("Crosstab button not found");
    btn.click();
  });
  ...
  await exportExcel(page, frame, "rejsekort_latest_year_daily_export.xlsx");
